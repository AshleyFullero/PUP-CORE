...
public class CreateStudentAccountController {
    ...
    public void initialize() {
        createBtn.setOnAction(event -> createStudentAccount());
    ...
    private void createStudentAccount() {
        ...
        String insertQuery = "INSERT INTO users (username, password, email, role) VALUES (?, ?, ?, 'student')";
        ...
        try (Connection connection = DBConnection.getConnection();
            ...
            if (rowsInserted > 0) {
                UtilityMethods.showPopup("Student account created successfully!");
                clearFields();
                UtilityMethods.switchToScene(createBtn, "login");
            } else {
                showAlert("Error", "Failed to create the student account.");
            }
